    ...
    return <div className="relative w-full h-16 ">
        ...
        <div className='flex flex-row h-full justify-between items-center px-4 backdrop-blur-sm'>
            <Link href="/" className=' flex flex-row gap-4 justify-start'>
                ...
                <h4 className=" font-shrikhand text-3xl text-white hover:text-green-600">NUHOC</h4>
            </Link>
            <div className=" hidden sm:flex flex-row gap-4 font-raleway">
                <Link href="/" className=" text-white hover:text-green-600">Home</Link>
                <Link href="/rent" className=" text-white hover:text-green-600">Gear</Link>
                <Link href="/account" className=" text-white hover:text-green-600">Membership</Link>
                <Link href={props.loggedIn ? "/api/auth/signout" : "/api/auth/signin"}
                    className=" text-white hover:text-green-600">{props.loggedIn ? 'Logout' : 'Login'}
                </Link>
            </div>
        </div>
    </div>
